createQualificationReport <- function(
  ...
  findRepoRoot <- function(path) {
    ...
    repeat {
      if (
        file.exists(file.path(current, "Clomiphene-Model.json")) &&
      ...
      if (identical(parent, current)) {
        stop(
          "Could not find repository root containing Clomiphene-Model.json and Evaluation",
          call. = FALSE
        )
      }
      current <- parent
  ...
  snapshotFile <- file.path(repoRoot, "Clomiphene-Model.json")
  ...
  dir.create(
    dirname(workflow$reportFilePath),
    recursive = TRUE,
    showWarnings = FALSE
  )
